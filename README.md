...
## Description
this code starts with configuring all clocks for used peripherals then send sentence to ESP **STARTFLASH** then a blocking uart section  will wait on buffer for about **4 secs** to receive **"OK"** to start flashing, otherwise it will jumb to firmware.
for flashing will start with takeing the length of the section that is going to be writen to flash then take data **252 bytes** each a multiple of **4** then flashes them to memory **4-bytes by 4-bytes** then sends "OK" indicating successful operation and so on till finishing .bin file then sends **0x00** for data length to jump to **user code (Firmware)**.
